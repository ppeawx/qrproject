// QR Code Generator Scripts
    ...
    if (!text) {
        ...
        Swal.fire({
            ...
            confirmButtonColor: "#667eea", // สีปุ่มที่ตรงกับดีไซน์ของคุณ
    ...
    downloadOptions.classList.add('show');
        ...
        function generateSVG(text, size, fgColor, bgColor, errorCorrection) {
            ...
            for (let y = 0; y < canvas.height; y++) {
                for (let x = 0; x < canvas.width; x++) {
                    ...
                    if (imageData[index] !== parseInt(bgColor.slice(1, 3), 16) || 
                        imageData[index + 1] !== parseInt(bgColor.slice(3, 5), 16) ||
                        imageData[index + 2] !== parseInt(bgColor.slice(5, 7), 16)) {
        ...
        sizeSlider.addEventListener('input', () => {
            ...
            debounce(generatePreviewQR, 300)();
            const downloadOptions = document.getElementById('download-options'); // <-- เพิ่มโค้ด
            downloadOptions.classList.remove('show'); // <-- เพิ่มโค้ด
        });
